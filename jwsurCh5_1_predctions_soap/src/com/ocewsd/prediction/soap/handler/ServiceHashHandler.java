...
public class ServiceHashHandler implements SOAPHandler<SOAPMessageContext> {
	...
	public ServiceHashHandler() {
		System.out.println("	New Instance: ServiceHashHandler ");
	}
	...
	public boolean handleMessage(SOAPMessageContext mCtx) {
		...
		if (!outbound) {
			try {
				...
				if(node == null){
					generateFault(msg, "Too few header blocks! (No child nodes)");
				}
		...
		System.out.println("\t Returning TRUE: continue down the handler chain");
		return true; 
	}
